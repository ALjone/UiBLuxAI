...
class TD:
    ...
    def _unit_mask_actions(self, outputs, state):
        mask = batched_action_mask(outputs, state)
    ...
    def predict(self, state, image_features, global_features):
            with torch.no_grad():
                self.model.eval()
                pred_units, pred_factories = self.model(image_features, global_features)
                unit_actions = self._unit_mask_actions(pred_units, state)
