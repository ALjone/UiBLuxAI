...
def transfer(res_type, x=1000, direction=None, repeat=0):
    ...
    if isinstance(res_type, int):
        return np.array([1, direction, res_type, x, repeat, 1])
    if res_type == "ice":
        return np.array([1, direction, 0, x, repeat, 1])
    elif res_type == "ore":
        return np.array([1, direction, 1, x, repeat, 1])
    elif res_type == "water":
        return np.array([1, direction, 2, x, repeat, 1])
    elif res_type == "metal":
        return np.array([1, direction, 3, x, repeat, 1])
    elif res_type == "power":
        return np.array([1, direction, 4, x, repeat, 1])
    else:
