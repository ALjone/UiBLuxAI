...
class PPO:
    def __init__(self, unit_action_dim, lr_actor, lr_critic, gamma, K_epochs, eps_clip, device = torch.device("cuda")):
        self.device = device
        self.gamma = gamma
        ...
        self.policy = ActorCritic(unit_action_dim).to(device)
        self.optimizer = torch.optim.Adam([
                        {'params': self.policy.actor.parameters(), 'lr': lr_actor},
                        {'params': self.policy.critic.parameters(), 'lr': lr_critic}
                    ])

        self.policy_old = ActorCritic(unit_action_dim).to(device)
    ...
    def select_action(self, state):
        ...
        with torch.no_grad():
            ...
            action_unit, action_logprobs_unit, state_values = self.policy_old.act(state)
    ...
    def update(self):
        ...
        for i in range(self.K_epochs):
            ...
            ratios = torch.exp(action_logprobs - old_logprobs_unit.detach())
