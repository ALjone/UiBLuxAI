...
class action_wrapper(gym.ActionWrapper):
    ...
    def transform_action(self, actions, player, obs):
        ...
        action = unit_output_to_actions(actions["unit_action"], units, state, obs["board"]["rubble"]) #Second channel is always factory_map

        action = action | factory_output_to_actions(actions["factory_action"], factories)
