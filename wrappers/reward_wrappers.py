...
class IceRewardWrapper(gym.RewardWrapper):
    ...
    def reward(self, rewards):
        ...
        self.previous_ice = sum([val for val in self.env.state.stats["player_0"]["generation"]["ice"].values()])
        self.previous_ore = sum([val for val in self.env.state.stats["player_0"]["generation"]["ore"].values()])
        ...
        if self.env.state.real_env_steps == 1000:
            ...
            end_of_game_reward += self.config["scaling_win"]*lichen_distribution
